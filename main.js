...
function _applySettingsSnapshot(snap) {
    ...
    // Live-apply только для настроек, у которых это безопасно (нет
    // дорогой инициализации, нет кэша в замыкании jQuery): тема и
    // размеры. values и rangesBounds трогают слайдеры с min/max,
    // закэшированными в jQuery.ready — пытаться их live-обновить
    // приводило к петле «init → диф → reload → init → …». Они
    // подхватятся при следующем перезаходе.
